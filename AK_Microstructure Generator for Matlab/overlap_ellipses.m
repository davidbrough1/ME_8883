...
% two centers is less than the minimum possible distance between the two
...
if overlap == -1;
    ...
    DIST = [X1-x0;Y1-y0]';
    NORM = sqrt(DIST(:,1).^2+DIST(:,2).^2);
    if min(NORM) < b1 + 10 ; overlap = 1; end;
    if min(NORM) > a1; overlap = 0; end;

    % Check distances for severe overlaps
    DIST = [X0-x1;Y0-y1]';
    NORM = sqrt(DIST(:,1).^2+DIST(:,2).^2);
    if min(NORM) < b0 + 10 ; overlap = 1; end;
